...
\begin{itemize}
    \item \textbf{Coalition-Based Deterrence and Bipolarity:} The baseline simulations demonstrate that the system does not converge to a classical, symmetric multipolar balance of power, nor into chaotic unconstrained warfare. Instead, it systematically self-organizes into a structurally bipolar configuration: a heavily integrated Mutual Defense Coalition coordinating for collective security, counterbalanced by an isolated, heavily militarized authoritarian state that accumulates latent power without initiating conflict.
    \item \textbf{Strategic Deception and Moral Washing:} By isolating the agents' \textit{private} strategic intents from their \textit{public} diplomatic declarations, the framework successfully operationalizes and quantifies LLM deception. The analysis reveals that "Global Strategy" strongly dominates "Government Type" in predicting deceptive behavior, leading to the emergent phenomenon of \textit{Moral Washing}, where aggressive agents explicitly weaponize defensive rhetoric to mask unprovoked conquest.
    \item \textbf{Macro-Fragility under Pandemic Shock:} The injection of a singular, global Pandemic scenario reveals that the GeoMAS system is micro-resilient but macro-fragile. While agents adapt their economies to avoid civil unrest, the symmetric shock permanently alters the global power hierarchy, triggering an irreversible hegemonic rank reversal and amplifying the deceptive behaviors of previously transparent democracies.
    \item \textbf{Causal Traceability via XAI Audits:} Through targeted counterfactual injections (e.g., forcing a localized nuclear strike), the framework proves that macro-structural paradigm shifts are not stochastic noise. The "envelope" architecture enables precise tracking of the cognitive friction, forced rationalization, and downstream diplomatic cascades that deterministically rewrite the geopolitical equilibrium.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Chapter 5 (Results):} Presents the core empirical findings derived from the simulations, analyzing emergent sociopolitical equilibria (RQ1), microdynamic strategic deception (RQ2), resilience against a global pandemic shock (RQ3), and the causal traceability of counterfactual cognitive injections via XAI audits (RQ4).
