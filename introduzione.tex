...
\begin{itemize}
    \item \textbf{Emergent Hegemonic Equilibria:} The baseline simulations demonstrate that the system does not converge to a classical, symmetric multipolar balance of power. Instead, it systematically self-organizes into a unipolar hegemonic structure. Agents employing a strategy of ``strategic patience'', deferring overt aggression while accumulating power internally and building diplomatic cohesion, significantly outperform those relying on unprovoked military expansion or strict isolationism.
...
Furthermore, this work introduces a structural methodology for causal verification through state-forking. Future extensions of the results will detail the macroscopic impacts of \textbf{exogenous scenario perturbations} (such as global pandemics, systemic insurrections, and localized resource discoveries) on the socio-political trajectory of the simulation. Additionally, through \textbf{counterfactual XAI injections}, the framework traces the precise causal chain of forced strategic deviations, demonstrating how a single localized cognitive constraint cascades to reshape the entire geopolitical balance.
...
\begin{itemize}
    ...
    \item \textbf{Chapter 5 (Results):} Presents the core empirical findings derived from the baseline simulations, analyzing emergent sociopolitical equilibria (RQ1) and microdynamic strategic deception (RQ2). It also evaluates the system's resilience to global shocks (RQ3) and the causal impacts of targeted counterfactual injections (RQ4).
