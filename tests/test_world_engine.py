...
def test_determinism():
    ...
    # Check deep equality of a random province (Fix: Don't assume ID 0 exists)
...
def test_spatial_manager_connectivity():
    ...
    # Note: n_cells is a target, actual valid Voronoi regions might be fewer due to filtering
    world = generate_world(seed=123, n_cells=200, n_nations=5)
    manager = SpatialManager(world)
    
    # Graph should have same number of nodes as VALID provinces generated
    assert len(manager.graph.nodes) == len(world.provinces)
    
    # Test pathfinding between neighbors
    p_id = list(world.provinces.keys())[0]
    
    # Ensure the province has neighbors (it might be an isolated island in edge cases, but unlikely with 200 cells)
    if world.provinces[p_id].neighbors:
        neighbor_id = world.provinces[p_id].neighbors[0]
        
        # Verify neighbor exists in world (sometimes Voronoi adjacency includes infinite regions which we filter out)
...
def test_spatial_translator_logic():
    ...
    # Check if it correctly identifies landlocked vs coastal
    ...
    if not coastal:
        ...
        # It could be Island or Maritime or Continental
        pass 
