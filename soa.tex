...
Conceptually, this episode can be read as a form of \textbf{meta-level negotiation protocol} between state institutions and AI laboratories. The Pentagon treats major model providers as strategic counterparts, leveraging procurement power, blacklisting threats, and regulatory influence to shape the effective action space of frontier LLMs, much like agents in a coalition game bargain over acceptable moves and red lines.\cite{moghimifar2024modelling} The labs, by contrast, attempt to encode normative constraints (no autonomous weapons, no mass surveillance) into their usage policies and technical architectures, effectively proposing an institutional ``Rules Oracle'' (in the sense of Section~\ref{sec:soa_limitations}) that sits above any concrete simulation or deployment. The breakdown with Anthropic, and the more flexible accommodation reached with OpenAI, show that this higher-level bargaining can reshape which cognitive architectures and safety mechanisms ultimately become embedded in real-world command-and-control pipelines.

Finally, just as Diella exemplifies a civilian MAS where LLM-based agents support anti-corruption and legislative analysis inside a national parliament (Section~\ref{sec:diella_real_world}), the Anthropic--OpenAI--Pentagon dispute demonstrates that LLM-powered systems are already being woven into the institutional fabric of national defense, intelligence, and war planning.\cite{treccaniAlbaniaArriva,indiatimesMeetWorlds,tazAlbanischesRegierungsmitglied} In both cases, frontier models move beyond sandboxed simulations towards roles in which their recommendations can influence the allocation of public resources, the interpretation of threats, and potentially the authorization of force. For the purposes of this thesis, this real-world evolution strengthens the motivation for a framework like GeoMAS: only architectures that combine deterministic physical-economic engines, formally specified action layers, and verifiable causal tracing can offer a credible safety and accountability substrate once LLM agents are no longer merely \textit{imagining} wars and crises, but advising the institutions that wage and govern them.
