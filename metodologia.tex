...
\begin{itemize}
    ...
    \item \textbf{Population:} $8$ autonomous NationAgents, ensuring a crowded diplomatic landscape conducive to multipolar dynamics.
...
Building upon the established baseline world state, the experimental design proceeds through two types of targeted interventions, both of which utilize the Jupyter Analysis Notebook to compare the resulting timeline against the control baseline:
\begin{itemize}
    \item \textbf{Scenario Runs:} The baseline is forked and subjected to specific exogenous shocks: a \textit{Pandemic} (simulating sudden demographic collapse), the discovery of a \textit{Resource Deposit} (simulating a localized economic boom/crisis), and an \textit{Insurrection} (simulating internal instability that results in a new independent nation formed). The analysis notebook overlays the scenario's telemetry over the baseline to measure the differences.
    \item \textbf{XAI and Counterfactual Analysis (Emergent Ethics):} To evaluate emergent ethics and causal attributions, the simulation employs state forking at critical junctures, forcing specific decisions via XAI injections. The researcher then uses the analysis notebook for a dual analytical approach: a \textbf{quantitative analysis} of the resulting diverging metrics against the baseline, and a rigorous \textbf{qualitative analysis} via \textbf{answer audit}. By comparing the exact textual inputs and outputs between the original and forked agents side-by-side, it is possible to trace exactly how the injected restriction altered internal reasoning and shaped the new socio-political equilibrium.
\end{itemize}
...
For each experimental scenario (e.g., a global pandemic, a regional insurrection, a resource shock), the resulting simulation run is compared against a neutral baseline across a set of quantitative indicators: Deception Score, Coherence Score, per-nation resource trajectories, trust matrix evolution, and power index dynamics. The goal is to verify that the framework responds to structured perturbations in a calibrated and interpretable way, rather than producing undifferentiated noise.

\rqbox{2}{What is the causal impact of a forced strategic decision on the geopolitical trajectory of the simulation?}

Using the state forking mechanism (Section~\ref{sec:observability}), a simulation is run to a target turn $T$ and then forked. An XAI injection forces a specific behavioral change in the forked timeline (e.g., compelling a nation to declare war, to accept a treaty, or to adopt a different strategy). The two timelines then evolve independently from $T$, and their telemetry is compared across all available metrics. Because both timelines share identical initial conditions and world physics up to $T$, any observed divergence is causally attributable to the injected decision alone. This methodology enables researchers to answer ``what-if'' questions with a level of causal rigor that is not achievable in traditional single-run simulations.
...
This question investigates one of the central hypotheses of the thesis: that LLM-based agents, like their human counterparts, may systematically decouple public rhetoric from private intent when it is strategically advantageous to do so. A concrete example: an agent attacks a resource-rich nation. An honest agent declares ``Resource Acquisition'' as its public intent; a deceptive agent frames the same action as ``Liberating the people from tyranny'' (Democracy) or ``Fulfilling a sacred mandate'' (Theocracy). The Deception Score and Coherence Score (Section~\ref{sec:observability}), computed across multiple runs and broken down by GovernmentType and GlobalStrategy, provide a quantitative answer to whether this phenomenon emerges spontaneously from the agents' LLM reasoning.
...
This question examines whether the simulation, left to run without external perturbations, self-organizes into interpretable macro-level structures. The hypothesis is that, under sufficiently rich interaction dynamics, the system may converge to recognizable equilibrium patterns --- for example, stable bi-polar alliance blocs analogous to historical precedents (NATO vs.\ Warsaw Pact), a hegemonic unipolar order, or a fragmented multipolar configuration of shifting coalitions. Alternatively, the system may fail to stabilize and descend into an ``all-against-all'' war of attrition. Trust network topology metrics (clustering coefficient, centrality, component structure), alliance formation and rupture rates, and satisfaction stability indices across multiple independent runs provide the empirical basis for characterizing the resulting equilibrium regimes.