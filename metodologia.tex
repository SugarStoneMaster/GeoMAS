...
\begin{enumerate}
    ...
    \item \textbf{Cognitive Layer (The ``Brain''):} A neuro-symbolic layer where nations are modeled as hierarchical cabinets. Three specialist Ministers propose actions concurrently via LLM calls; the President LLM synthesizes proposals through approval or veto. A separate Public Opinion agent reacts post-execution, modulating satisfaction dynamics.
    ...
    \item \textbf{Observability Layer (The ``Observer''):} An analytical meta-layer responsible for both \textit{Persistence} (state snapshots, agent traces, token usage via DuckDB) and \textit{Behavioral Intelligence} (deception scoring, coherence analysis, counterfactual divergence measurement).
...
\begin{itemize}
    ...
    \item \textbf{The Public Opinion (The Internal Adversary):} Unlike the Ministers, this agent operates \textit{post-execution}. Its prompt is injected with cultural trait data (e.g., ``Nationalist, 1.3x military sensitivity''), and it returns multipliers (0.1--2.0) that modulate how satisfaction changes are applied. A deterministic fallback activates when the LLM is unavailable.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Reaction Phase:} The Public Opinion agent evaluates the resulting state changes. Satisfaction deltas are calculated from events, government actions, and current state, modulated by the LLM's cultural multipliers. If satisfaction drops below critical thresholds, automatic triggers fire (General Strike, Civil Unrest).
\end{enumerate}
...
Cultural traits generated during world creation (e.g., Nationalist, Pacifist, Isolationist) modulate the LLM-generated multipliers (0.1--2.0$\times$) applied to satisfaction changes, allowing each nation's population to react differently to identical stimuli.
...
To answer causal questions such as ``What would have happened if Nation~A had not declared war?'', GeoMAS implements a \textbf{fork-and-compare} mechanism:
\begin{enumerate}
    ...
    \item The forked simulation diverges from the base timeline as the injected instruction alters agent behavior.
    \item A \textit{DeltaAnalyzer} computes per-nation divergence (satisfaction, power, resources, military) and global divergence (trust shifts, relationship changes) between the two timelines, producing a weighted divergence score.
    \item An LLM-powered explanation generates a causal narrative linking the injection to the observed deltas.
\end{enumerate}

This method provides verifiable counterfactual explanations: the divergence is grounded in the deterministic simulation physics, not in post-hoc rationalization by the LLM.
