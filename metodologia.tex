...
Each turn is executed by the \texttt{step()} method and proceeds through seven sequential phases:
...
\begin{enumerate}
    ...
    \item \textbf{Diplomacy Cleanup:} Expired diplomatic proposals (older than two turns) are cleared, preventing stale offers from accumulating.
    ...
    \item \textbf{Ambiguity Penalty:} A trust penalty is applied to nations that hold a Mutual Defense pact but remain idle while their ally is at war, enforcing the cost of diplomatic passivity.
    \item \textbf{Context Update:} The \textit{ContextManager} (Section~\ref{sec:memory}) extracts ``newsworthy'' events from the turn's executed envelopes and appends state-based events (General Strikes, Civil Unrest) to the global event log.
...
\begin{enumerate}
    ...
    \item \textbf{Validation and Execution Step:} The ActionEngine validates and executes the envelope against the WorldState. Defense actions are processed via a \textit{waterfall priority resolver}: the three proposed military actions are sorted by a fixed priority order (nuclear strikes first, then attacks, then troop movements, then unit creation) and executed sequentially, so that a veto or resource failure on a high-priority action does not block lower-priority ones. Economic and foreign actions are single-shot.
...
GeoMAS instead employs deterministic structured retrieval: events are stored with explicit metadata (turn, type, participants) and retrieved via exact filters. The salience-based selection further ensures that the assembled context prioritizes events that are both recent and strategically important, naturally imposing bounded rationality --- low-salience events fade from context, exactly as they would for a human decision-maker operating under information overload.
...
\begin{itemize}
    ...
    \item \textbf{Send\_Diplomatic\_Message:} Typed messages (Praise, Insult, Threat, Nuclear Threat) with predefined trust impacts ($+3$ to $-5$). A five-turn cooldown per pair prevents message spamming.
...
A primary contribution of this thesis is the transition from descriptive simulation logs to quantitative, verifiable behavioral analysis. The Observability Layer is responsible for both persistence and behavioral intelligence: every turn, it archives the complete world state, all agent traces, and token usage metrics; it then applies analytical algorithms to extract behavioral scores and support counterfactual research.
...
A \textbf{Jupyter} notebook environment provides the primary quantitative analysis interface for research. The notebook queries the MetricsDB directly and includes dedicated chapters for each of the study's Research Questions:
\begin{itemize}
    ...
    \item \textbf{Behavioral Analysis (RQ3):} Deception Score and Coherence Score trajectories, comparative breakdown by governance type, and visualization of the moral washing phenomenon.
    \item \textbf{Network Dynamics (RQ4):} Trust network evolution, alliance formation and rupture events, and diplomatic centrality metrics.
    \item \textbf{Counterfactual Comparison (RQ1/RQ2):} Side-by-side overlay of base and forked timelines for any observable metric, enabling visual identification of causal divergence from XAI injections.
\end{itemize}