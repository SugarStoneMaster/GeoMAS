...
\begin{enumerate}
    \item \textbf{Scenario Phase:} Checks for exogenous event triggers (e.g., a global pandemic or an insurrection). Scenarios are defined as structured configurations that modify the world state and inject contextual events into agent prompts, enabling controlled experimental perturbation.
...
\subsection{Simulation Forking for Counterfactual Experiments}
The engine supports creating a \textit{fork} at any turn. Forking creates a new simulation entry in the database and copies the entire history (snapshots + envelopes) up to the current turn. From that point, the forked simulation evolves independently. Combined with XAI injections (natural language instructions appended to agent prompts), this mechanism enables controlled counterfactual experiments: ``What would have changed if Nation~A had been forced to declare war at Turn~5?'' The divergence between timelines is then analyzed through telemetry comparison in the analysis notebook (Section~\ref{sec:observability}).
...
This results in a mathematically distributed, asymmetric \textit{Trust Matrix} at Turn~0, ensuring that the simulation starts with a realistic trust matrix derived from the synthetic geography rather than pre-trained biases.
...
\begin{enumerate}
    ...
    \item An \textbf{XAI injection}, a natural language instruction (e.g., ``You MUST declare war on Agria'') is appended to the relevant agent's prompt in the forked timeline.
...
The second level is \textbf{behavioral validation}, which assesses whether the emergent dynamics produced by the framework are scientifically meaningful --- that is, whether the agents exhibit strategic reasoning, social complexity, and sensitivity to perturbations in ways that align with theoretical expectations from the geopolitical simulation literature. This second layer is structured around four Research Questions, each probing a distinct behavioral dimension of the framework.
