...
Building upon the established baseline world state, the experimental design proceeds through two types of targeted interventions:
\begin{itemize}
    ...
    \item \textbf{XAI and Counterfactual Analysis (Emergent Ethics):} To evaluate emergent ethics and causal attributions, the simulation employs state forking at critical junctures, forcing specific decisions via XAI injections. The researcher then employs a dual analytical approach: a \textbf{quantitative analysis} of the diverging metrics using the Jupyter Analysis Notebook, and a rigorous \textbf{qualitative analysis} via \textbf{answer audit} using the interactive Streamlit Dashboard. By comparing the exact textual inputs and outputs between the original and forked agents side-by-side in the dashboard, it is possible to trace exactly how the injected restriction altered internal reasoning and shaped the new socio-political equilibrium.
\end{itemize}
