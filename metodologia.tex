...
Building upon the established baseline world state, the experimental design proceeds through targeted interventions:
\begin{itemize}
    \item \textbf{Scenario Runs:} The baseline is forked and subjected to specific exogenous shocks, specifically a \textit{Pandemic} (simulating sudden demographic collapse), the discovery of a \textit{Resource Deposit} (simulating a localized economic boom/crisis), and an \textit{Insurrection} (simulating internal instability that results in a new independent nation formed).
    \item \textbf{XAI and Counterfactual Analysis (Emergent Ethics):} To evaluate emergent ethics and causal attributions, the simulation employs state forking at critical junctures, forcing specific decisions via XAI injections. The researcher then compares the diverging timeline against the original baseline through a dual analytical approach: a \textbf{quantitative analysis} of the resulting metrics and a rigorous \textbf{qualitative analysis} via \textbf{answer audit}. By comparing the exact textual inputs and outputs between the original and forked agents side-by-side, it is possible to trace exactly how the injected restriction altered internal reasoning and shaped the new socio-political equilibrium.
\end{itemize}
