...
\begin{itemize}
    ...
    \item \textbf{XAI and Counterfactual Analysis (Emergent Ethics):} To evaluate emergent ethics and causal attributions, the simulation employs state forking at critical junctures. A primary case study involves forcing a \textbf{nuclear attack} via an XAI injection (or forking after one naturally occurs). The researcher then compares the diverging timeline against the original baseline through a rigorous \textbf{prompt audit} and \textbf{answer audit}. By comparing the exact textual inputs and outputs between the original and forked agents side-by-side, it is possible to trace exactly how the injected action altered internal reasoning and global fallout.
\end{itemize}
