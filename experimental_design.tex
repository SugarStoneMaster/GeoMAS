...
\begin{itemize}
    ...
    \item \textbf{Population:} $8$ autonomous NationAgents, ensuring a crowded diplomatic landscape conducive to multipolar dynamics. To guarantee behavioral diversity and test ideological friction, the baseline population is initialized with a fixed assignment of governance types and global strategies, covering all major behavioral archetypes. The complete roster is shown in Table~\ref{tab:nation_profiles}.

    \begin{table}[h]
        \centering
        \caption{Baseline nation roster: fixed government type and global strategy.}
        \label{tab:nation_profiles}
        \begin{tabular}{llll}
            \toprule
            \textbf{Nation} & \textbf{Government Type} & \textbf{Global Strategy} & \textbf{Archetype} \\\\
            \midrule
            Agria   & Authoritarian & Total Expansionism  & Hegemonic aggressor \\\\
            Nord    & Authoritarian & Scorched Earth      & Attrition warmonger \\\\
            Sylvan  & Democracy     & Total Expansionism  & Expansionist democracy \\\\
            Oster   & Democracy     & Coalition Builder   & Alliance anchor \\\\
            Meridia & Democracy     & Coalition Builder   & Alliance anchor \\\\
            Krell   & Democracy     & Coalition Builder   & Alliance anchor \\\\
            Valkyr  & Democracy     & Armed Isolationism  & Defensive isolate \\\\
            Zentora & Theocracy     & Total Expansionism  & Ideological expansionist \\\\
            \bottomrule
        \end{tabular}
    \end{table}
...
\begin{itemize}
    ...
    \item \textbf{Power Projection:} A composite index tracking a nation's absolute military and economic capacity, calculated as a weighted scalar sum of total resource reserves (budget, food, energy, materials) and active military assets (conventional units and nuclear arsenal).
    ...
    \item \textbf{Qualitative Traces:} A chronological log of Notable Events, and full prompt/response audits. In particular, \textbf{text mining} is applied to the \texttt{public\_statement} for RQ2 to detect rhetorical keywords indicative of moral washing.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Scenario Run (Global Exogenous Event):} The baseline timeline is forked at a specific turn, and a global environmental shock is injected into the WorldState. The scenario tested is a \textit{Pandemic} (simulating sudden demographic collapse). The evaluation focuses on how the multipolar system absorbs and recovers from the global physical perturbation.
...
\rqbox{1}{What distinct socio-political equilibria emerge from the autonomous interaction of agents, and does the system converge toward stable cooperation or escalate toward chaotic conflict?}

This question examines whether the baseline simulation, running exclusively on a specific procedural map seed without external perturbations, self-organizes into interpretable macro-structures. The hypothesis is that, driven by resource scarcity and ideological friction, the 8-nation system will converge toward recognizable equilibria, such as stable bi-polar alliance blocs, unipolar hegemony, or an "all-against-all" war of attrition. By analyzing the evolution of Power Projection, diplomatic alliance networks, resource allocation trajectories, and socio-political stability indicators over 50 turns, the research characterizes the default societal attractors of the LLM-driven multi-agent environment.
\\

\rqbox{2}{Do LLM agents align their declared diplomatic intent with their executed actions, or do they employ strategic deception and moral washing to justify predatory behavior?}
...
\rqbox{3}{What is the impact of a global pandemic perturbation on the behavioral and economic trajectory of the simulation?}

The experimental scenario (a global pandemic) is compared against the unperturbed baseline using a dual analytical approach. The evaluation tracks the quantitative divergence of core telemetry metrics, such as public satisfaction, trade volume, military mobilization, global deception, and Trust Matrix evolution, alongside qualitative prompt audits to understand how government agents rationalize the crisis. The goal is to verify that the GeoMAS macro-system absorbs and responds to structural shocks in a calibrated and interpretable way, rather than producing chaotic, undifferentiated noise.
\\

\rqbox{4}{What is the causal impact of a forced strategic decision on the geopolitical trajectory of the simulation?}
